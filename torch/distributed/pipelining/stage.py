...
class _PipelineStageBase(ABC):
    ...
    def _validate_bwd_inputs(self, output_grads: tuple[Optional[torch.Tensor], ...]):
    ...
    def _validate_bwd_outputs(self, input_grads: tuple[Optional[torch.Tensor], ...]):
            ...
            if not info.is_root_arg:
                ...
                if isinstance(info.tensor_meta, _DTensorMeta):
                    ...
                    activation = DTensor.from_local(
                        ...
                        run_check=False,
                    ).requires_grad_(info.tensor_meta.requires_grad)
                    ...
                    activation = local_tensor.requires_grad_(
...
class _PipelineStage(_PipelineStageBase):
    ...
    def _extract_input_metas_from_graph(self) -> tuple[TensorMeta, ...]:
        """
        Extract input tensor metadata from graph placeholders.

        Note: DTensors are NOT supported in the traced frontend.
        Use the manual PipelineStage frontend for DTensor support.
        """
        placeholders = [
            node
            for node in self.submod.graph.nodes  # type: ignore[arg-type,union-attr]
        ...
        for placeholder in placeholders:
            ...
            if isinstance(example_value, DTensor):
                raise PipeliningMetadataError(
                    f"{self.log_prefix} DTensor detected in traced pipeline input '{placeholder.name}'. "
            ...
            input_metas.append(_TensorMeta.from_tensor(example_value))

        return tuple(input_metas)
    ...
    def _prepare_forward_infra(
        ...
        Extracts input/output metadata from the traced graph.
        """
        # Extract input metadata from graph placeholders
        self._stage_meta.inputs = self._extract_input_metas_from_graph()

        # Create recv info for each microbatch
        for chunk in range(num_microbatches):
            self.args_recv_info[chunk] = self._create_act_recv_info()

        # Create send info and extract output metadata
        # Note: _create_act_send_info already sets self._stage_meta.outputs
    ...
    def _create_act_recv_info(
        ...
        if self._stage_meta.inputs is None:
            raise PipeliningMetadataError(
                f"Stage {self.stage_index}: _stage_meta.inputs must be populated before "
                f"creating recv info. Call _extract_input_metas_from_graph first."
            )
        ...
        def create_recv_tensor(placeholder, arg_node, input_idx: int):
            ...
            if arg_node.op == "placeholder":
                ...
                tensor_meta = (
                    inputs_meta[input_idx] if input_idx < len(inputs_meta) else None
                )
                return _RecvInfo(
            ...
            if self.has_backward and tensor_meta.requires_grad:
...
class PipelineStage(_PipelineStageBase):
    ...
    def _forward_metadata_inference(
        ...
        grad_context = torch.no_grad() if not has_backward else nullcontext()

        # Run forward on meta tensors
        # We temporarily convert module parameters/buffers to meta device to allow
        # operations between meta input tensors and module weights. This handles
        # both plain tensors and DTensors (for tensor parallel models).
